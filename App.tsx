...
const App: React.FC = () => {
  ...
  const deleteMedicine = (id: string) => {
    if (window.confirm("Are you sure you want to delete this medicine?")) {
        ...
        // Remove associated logs so it clears from Weekly Report
        setAllLogs(prev => prev.filter(l => l.medicineId !== id));
  ...
  const addLog = useCallback((medicineId: string, status: 'taken' | 'missed', ownerId?: string) => {
    const cid = ownerId || currentUser?.caretakerPhone;
    if (!cid) return;

    // Prevent duplicates for the same day and medicine
    const todayStr = new Date().toDateString();
    const isDuplicate = allLogs.some(l => 
        l.medicineId === medicineId && 
        l.timestamp.toDateString() === todayStr
    );

    if (isDuplicate) {
        // Log exists, just close the modal
        setActiveReminder(null);
        return;
    }
  ...
  }, [activeReminder, lastSmsTime, allUsers, addLog]);
